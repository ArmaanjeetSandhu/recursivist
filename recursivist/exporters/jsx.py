    ...
    def export(self, output_path: str) -> None:
        def _build_structure_jsx(
            ...
            for name, content in sorted(
                ...
                if path_parts[0] == self.root_name and len(path_parts) > 1:
                    path_parts = [self.root_name] + [p for p in path_parts[1:] if p]
                else:
                    path_parts = [p for p in path_parts if p]
                    if not path_parts or path_parts[0] != self.root_name:
                        path_parts = [self.root_name] + path_parts
            ...
            if "_files" in structure:
                ...
                for entry in sorted_files:
                    ...
                    if path_prefix:
                        ...
                        if path_parts and path_parts[0] == self.root_name:
                            path_parts = [self.root_name] + [
                                p for p in path_parts[1:] if p
                            ]
                        else:
                            path_parts = [p for p in path_parts if p]
                            if not path_parts or path_parts[0] != self.root_name:
      ...
      const format_timestamp = (timestamp) => {
        ...
        const DirectoryItem = (props) => {{
          ...
          useEffect(() => {{
            if (collapseAll && folderId !== '{html.escape(self.root_name)}') {{
              setOpenFolders(prev => {{
              ...
              <div className={{`min-h-screen ${{darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-900'}}`}}>
                ...
                <div className="container mx-auto max-w-5xl p-4">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6">
                    <div>
                      <h1 className="text-xl font-bold flex items-center">
                        ...
                        Directory Structure: {html.escape(self.root_name)}
                      </h1>
                  ...
                  <div className={{`p-4 rounded-lg shadow ${{darkMode ? 'bg-gray-800' : 'bg-white'}}`}}>
                    <DirectoryItem
                      name="{html.escape(self.root_name)}"
                      level={{0}}
                      path={{["{html.escape(self.root_name)}"]}}{root_loc_prop}{
            root_size_prop
        }{root_mtime_prop}
                    >
        {
            _build_structure_jsx(
                self.structure, 1, self.root_name if self.show_full_path else ""
            )
        }
                    </DirectoryItem>
